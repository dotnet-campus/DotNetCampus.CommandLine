...
public class OptionValueSeparatorTests
{
    ...
    [TestMethod]
    // option=value
    [DataRow(new[] { "--option=value" }, TestCommandLineStyle.Posix, DisplayName = "[Posix] --option=value")]
    // o=value
    [DataRow(new[] { "-o=value" }, TestCommandLineStyle.Posix, DisplayName = "[Posix] -o=value")]
    // option:value
    [DataRow(new[] { "--option:value" }, TestCommandLineStyle.Gnu, DisplayName = "[Gnu] --option:value")]
    [DataRow(new[] { "test://?option:value" }, TestCommandLineStyle.Url, DisplayName = "[Url] option:value")]
    // option value
    ...
    public void ShorOptionNotSupported(string[] args, TestCommandLineStyle style)
    {
        // Arrange
        var commandLine = CommandLine.Parse(args, style.ToParsingOptions());
