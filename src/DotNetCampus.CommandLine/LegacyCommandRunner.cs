...
/// <summary>
/// 辅助 <see cref="LegacyCommandLine"/> 根据已解析的命令行参数执行对应的命令处理器。
/// </summary>
public class LegacyCommandRunner : ILegacyCommandRunnerBuilder, ILegacyAsyncCommandRunnerBuilder
{
    ...
    private readonly LegacyCommandLine _commandLine;
    ...
    internal LegacyCommandRunner(LegacyCommandLine commandLine)
    {
        _commandLine = commandLine;
    }

    internal LegacyCommandRunner(LegacyCommandRunner commandRunner)
    {
    ...
    public static void Register<T>(string? commandNames, LegacyCommandObjectCreator creator)
        where T : class
    {
    ...
    internal static T CreateInstance<T>(LegacyCommandLine commandLine)
    {
    ...
    /// <typeparam name="T">命令处理器的类型。</typeparam>
    /// <returns>命令处理器实例。</returns>
    internal static T CreateInstance<T>(LegacyCommandLine commandLine, LegacyCommandObjectCreator creator)
    {
        return (T)creator(commandLine);
    }

    LegacyCommandRunner ILegacyCoreCommandRunnerBuilder.GetOrCreateRunner() => this;
    ...
    internal LegacyCommandRunner AddHandler<T>()
    ...
    /// <returns>返回一个命令处理器构建器。</returns>
    [EditorBrowsable(EditorBrowsableState.Never)]
    internal LegacyCommandRunner AddHandler<T>(string? command, LegacyCommandObjectCreator creator)
        where T : class, ICommandHandler
    {
    ...
    /// <typeparam name="T">命令处理器的类型。</typeparam>
    /// <returns>返回一个命令处理器构建器。</returns>
    internal LegacyCommandRunner AddHandler<T>(Func<T, Task<int>> handler)
        where T : class
    {
        if (!CommandObjectCreationInfos.TryGetValue(typeof(T), out var info))
        {
    ...
    /// <typeparam name="T">命令处理器的类型。</typeparam>
    /// <returns>返回一个命令处理器构建器。</returns>
    internal LegacyCommandRunner AddHandler<T>(string? command, LegacyCommandObjectCreator creator, Func<T, Task<int>> handler)
        where T : class
    {
    ...
    internal LegacyCommandRunner AddHandlers<T>()
    ...
    private readonly record struct CommandObjectCreationInfo(string? CommandNames, LegacyCommandObjectCreator Creator);
}
