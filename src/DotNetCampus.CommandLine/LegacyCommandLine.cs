...
/// 为应用程序提供统一的命令行参数解析功能。
/// </summary>
public class LegacyCommandLine : ILegacyCoreCommandRunnerBuilder
{
    /// <summary>
    /// 获取此命令行解析类型所关联的命令行参数。
    /// </summary>
    ...
    public static LegacyCommandLine Parse(IReadOnlyList<string> args, CommandLineParsingOptions? parsingOptions = null)
    {
        ...
        return args.Count is 0
            ...
            : new LegacyCommandLine(args, parsingOptions);
    }
    ...
    LegacyCommandRunner ILegacyCoreCommandRunnerBuilder.GetOrCreateRunner() => new(this);
    ...
    [Pure]
    public T As<T>() where T : class => LegacyCommandRunner.CreateInstance<T>(this);
    ...
    public T As<T>(LegacyCommandObjectCreator creator) where T : class => LegacyCommandRunner.CreateInstance<T>(this, creator);
