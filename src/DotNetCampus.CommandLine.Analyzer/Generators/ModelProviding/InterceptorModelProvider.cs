...
internal static class InterceptorModelProvider
{
    public static IncrementalValuesProvider<InterceptorGeneratingModel> SelectCommandLineAsProvider(this IncrementalGeneratorInitializationContext context)
    {
        return SelectMethodInvocationProvider(context,
            "DotNetCampus.Cli.Temp40.CommandLine", "As");
    }

    public static IncrementalValuesProvider<InterceptorGeneratingModel> SelectCommandBuilderAddHandlerProvider(
        this IncrementalGeneratorInitializationContext context)
    {
        return SelectMethodInvocationProvider(context, "DotNetCampus.Cli.Temp40.CommandRunnerBuilderExtensions", "AddHandler");
    }
    ...
    {
        return SelectMethodInvocationProvider(context,
            $"DotNetCampus.Cli.Temp40.{extensionMethodThisTypeName}", "AddHandler",
    ...
    {
        return context.SyntaxProvider.CreateSyntaxProvider((node,
                    ct) =>
                {
                    ...
                    ct) =>
                {
                    ...
                    // 确保此方法是 DotNetCampus.Cli.Temp40.CommandLine.As<T>() 方法（类也要匹配）。
                    var methodSymbol = ModelExtensions.GetSymbolInfo(c.SemanticModel,
                            node,
                            ct)
                    ...
                    for (var i = 0;
                         i < parameterTypeFullNameRegexes.Length;
                         i++)
                    {
                        var parameterSymbol = methodSymbol.Parameters[i];
                        if (!Regex.IsMatch(parameterSymbol.Type.ToGlobalDisplayString(),
                                parameterTypeFullNameRegexes[i]))
                        {
                            // 参数类型不匹配。
                            return null;
                        }
                    ...
                    var symbol = ModelExtensions.GetSymbolInfo(c.SemanticModel,
                            genericTypeNode,
                            ct)
                        .Symbol as INamedTypeSymbol;
                    var interceptableLocation = c.SemanticModel.GetInterceptableLocation(node,
                        ct);
                    ...
                    var commandAttribute = symbol.GetAttributes()
                                               .FirstOrDefault(a => a.AttributeClass!.IsAttributeOf<CommandAttribute>())
#pragma warning disable CS0618 // 类型或成员已过时
                                           ?? symbol.GetAttributes()
                                               .FirstOrDefault(a => a.AttributeClass!.IsAttributeOf<VerbAttribute>())
#pragma warning restore CS0618 // 类型或成员已过时
                        ;
                    var commandNames = commandAttribute?.ConstructorArguments.FirstOrDefault() is
                    {
                    ...
                    return new InterceptorGeneratingModel(interceptableLocation,
                        symbol,
                        commandNames,
                        invocationInfo);
                })
